...
JsonReader::JsonReader(const std::string& path){
  ...
  if (!root_.is_null()) {//get layers info
    ...
    for (auto& el_c : layers.items()) {
      ...
      AniInfoManager::GetIns().AppendLayerInandOut(layers_.back()->GetLayerInd() - 1, //after need to change :AE layer_index is start form 0
        layers_.back()->GetLayerInpos(), layers_.back()->GetLayerOutpos());
    }
  }
