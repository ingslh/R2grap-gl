...
public:
  PathInfo(const nlohmann::json& json): shape_direction_(json["Shape Direction"]), closed_(json["Closed"])
  {
    ...
    for(auto ind = 0; ind < outpos_list.size(); ind++){
      out_pos_vec.emplace_back(glm::vec2(outpos_list[ind][0], outpos_list[ind][1]));
      in_pos_vec.emplace_back(glm::vec2(inpos_list[ind][0], inpos_list[ind][1]));
      vertices.emplace_back(glm::vec2(vert_list[ind][0], vert_list[ind][1]));
    }
    if(closed_){
  ...
  const std::vector<glm::vec2>& GetOutPosVec() const {return out_pos_vec;}
  const std::vector<glm::vec2>& GetInPosVec() const {return in_pos_vec;}
